...
module ColorMath =
    ...
    let shiftColor (color: SKIDColor) (diff: SKIDColor) =
        SKIDColor(clamp (color.r + diff.r), clamp (color.g + diff.g), clamp (color.b + diff.b), clamp (color.a + diff.a))

    let applyMapping (source: SKIDColor[]) (target: SKIDColor[]) =
        let avgS = averageColor source
        let avgT = averageColor target
        let diff = SKIDColor( avgT.r - avgS.r, avgT.g - avgS.g, avgT.b - avgS.b, avgT.a - avgS.a )
    ...
    let GetMoodColor (source: SKIDColor[]) (k:int) =
        ...
        let diff = SKIDColor( domColor.r - 0.5f, domColor.g - 0.5f, domColor.b - 0.5f, 0.0f ) // 알파는 그대로
        source |> Array.map (fun c -> shiftColor c diff)
